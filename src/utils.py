    ...
    try:
        with open(filepath, 'r', encoding='utf-8') as f:
            ...
            if isinstance(data, list):
                utils_logger.info(f"Файл {filepath} успешно прочитан. Записей: {len(data)}")
                return data
            else:
                utils_logger.warning(f"Файл {filepath} не содержит список данных")
                return []
        ...
        utils_logger.error(f"Файл {filepath} содержит некорректный JSON")
        return []
    except Exception as e:
        utils_logger.error(f"Ошибка при чтении файла {filepath}: {str(e)}")
        return []